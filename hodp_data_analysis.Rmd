...
```{r}
df  %>% mutate(Culture=fculture) %>% ggplot(aes(x=accessionyear, y = views)) + geom_point(aes(color = Culture)) + ggtitle("Views and Accession Year") + xlab("Accession Year") + ylab("Average Views") + xlim(1890, 2020) + theme_hodp() +
#+scale_color_manual(name = "Tweet Type")
  #scale_x_date(date_minor_breaks = "7 days")  +
  #geom_smooth(aes(color = fculture), se = FALSE) +
  ggsave("accYear_views.png", width = 7.5, height = 5)
```
...
culturerank <- names(which(table(df$culture) > 50))
print(culturerank)
df20 = df %>% filter(fculture %in% culturerank) %>% filter(!(as.character(fculture) %in% c("Unknown")))
...
# dat.gg <- cbind(dat.gg, as.data.frame(circleData$avgViews))
# names(word_list)[4] = 'avgViews'
print(dat.gg)
circles = ggplot() + 
  
  # Make the bubbles
  geom_polygon(data = dat.gg, aes(x, y, group = id, fill="#E9141D"), colour = "black") +
  ...
  scale_size_continuous(range = c(1,4)) +
   ...
   labs(caption = "Source: https://harvardartmuseums.org/collections/api") + 
  ggtitle("Mean Views of HAM Works, by Culture") + 
  ...
  ggsave("culture_views_cloud.png")
  #coord_equal()
...
ggplotly(circles,tooltip = c("avgViews")) %>% layout(title = list(text = paste0('Mean Views of HAM Works, by Culture',
                                    '<br>',
                                    '<sup>',
                                    'Source: https://harvardartmuseums.org/collections/api',
                                    '</sup>')))
...
df$ffculture <- factor(df$ffculture, 
    levels = c("Japanese", "Other", "Italian", "British",  "Greek", "French", "German", "Roman","American", "Unknown"))
ggplot(df, aes(x=ffculture, y=rank)) +
  geom_violin() + labs(y = "Rank", x = "Culture",
  title = "Boxplots of Culture Ranks")
